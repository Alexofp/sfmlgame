...
void main()
{
    ...
    vec3 pixel1 = texture2D(texture1, gl_TexCoord[0].xy / 0.1 ).rgb;
	vec3 pixel2 = texture2D(texture2, gl_TexCoord[0].xy / 0.25 ).rgb;
	vec3 pixel3 = texture2D(texture3, gl_TexCoord[0].xy / 0.05 ).rgb;//texture2D(texture3, gl_TexCoord[0].xy / scale).rgb;
	
	vec4 pixel1z = texture2D(texture1z, gl_TexCoord[0].xy / 0.1 );
	vec4 pixel2z = texture2D(texture2z, gl_TexCoord[0].xy / 0.57 );
